...
boxplot(mtcars$mpg ~ mtcars$cyl, main = "Plot 4")
...
```{r}
libraies<-unique(Library_Language_Statistics$Library)
plots <- list()
top_label <- c('Total<br>Words', 'Translated', 'Fuzzy', 'Untranslated')
for(libr in libraies)
{
              ...
              marker = list(color = 'rgba(38, 24, 74, 0.8)',
                    ...
                    showlegend = TRUE) 
...
fi <- fi %>% add_annotations(xref = 'paper', yref = 'y', x = 0.14, y = df$Language,
                             xanchor = 'right',
                             text = df$Language,
...
fi <- fi %>% add_annotations(xref = 'x', yref = 'y',
                             x = df$Total_words / 2, y = df$Language,
                             ...
                             showarrow = FALSE) 
                               ...
                               x = df$Total_words+df$Translated / 2, y = df$Language,
                             ...
                             x = df$Total_words+df$Translated+df$Fuzzy / 2, y = df$Language,
                             ...
                             x = df$Total_words+df$Translated+df$Fuzzy+df$Untranslated / 2, y = df$Language,
...
subplot(plots, nrows =length(libraies)/2 , shareX = TRUE, titleX = FALSE)
