...
```{r}
  ...
  fig1 <- plot_ly(Language_Statistics_new, y = ~Population, x = ~Name, name = 'Population', type = 'scatter', mode = 'markers')
  fig2 <- plot_ly(Language_Statistics_new, y = ~Translated, x = ~Name, name = 'Translation', type = 'scatter', mode = 'markers')
  fig <- subplot(fig1,fig2,nrows=2,margin = 0.07,shareX = TRUE)%>%
    layout(title = list(text = "Population v/s Translations Completed"), plot_bgcolor='#e5ecf6', xaxis = list(title="Language",zerolinecolor = '#ffff', zerolinewidth = 2, gridcolor = 'ffff'), yaxis = list(title="Population",zerolinecolor = '#ffff', zerolinewidth = 2, gridcolor = 'ffff'),yaxis2=list(title="Translations"),showlegend=FALSE)
...
subplots2 <- subplot(plots, nrows = length(libraries) / 2) %>% layout(width = 1030, height = 305 * length(libraries))
scrollable_div <- div(
...
plot_r<-plot_ly(r_df,x=~libra,y=~total_word,name="Total words",type = "scatter",mode="lines+markers")
plot_r<-plot_r%>%add_trace(y=~translate,name="Translate",mode="lines+markers")%>%layout(showlegend = FALSE,annotations = list(
  list(
    x = 0.5,
    y = 1.1,
    text = "R component",
...
plot_c<-plot_ly(c_df,x=~libra,y=~total_word,name="Total words",type = "scatter",mode="lines+markers")
...
subplot(plot_r,plot_c,plot_eng,nrows=3,margin = 0.07)
